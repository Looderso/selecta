    ...
    def _on_platform_clicked(self, platform: str) -> None:
        ...
        if platform_value:
            ...
            for i in range(self.platform_layout.count()):
                button = self.platform_layout.itemAt(i).widget()
...
class TrackDetailsPanel(QWidget):
    ...
    def _fetch_platform_info(self, track: TrackItem) -> None:
        ...
        try:
            ...
            for platform in ["spotify", "discogs", "youtube", "rekordbox"]:
                ...
                if info:
                    ...
                    logger.info(
                        f"Found platform info for {platform}: platform_id={info.platform_id}"
                    )
    ...
    def _create_metadata_fields(self, track: TrackItem) -> None:
        """Create metadata fields from track data.
    ...
    def _add_platform_values_to_fields(
        self, platform_name: str, platform_metadata: dict[str, Any]
    ) -> None:
    ...
    def _on_apply_changes(self) -> None:
        ...
        try:
            ...
            has_image_changes = "cover_image" in self._changed_fields and hasattr(
            ...
            if has_image_changes:
                ...
                if cover_data and "pixmap" in cover_data and "metadata" in cover_data:
                    ...
                    success = self._save_cover_image(
                        track, cover_data["pixmap"], cover_data["metadata"]
                    )
                    if not success:
    ...
    def _load_track_images(self, track: TrackItem) -> None:
        ...
        same_track_with_saved_image = (
            self._image_just_saved and self._saved_image_track_id == track.track_id
        )
    ...
    def _add_platform_covers(self) -> None:
        ...
        for platform_name, images in platform_images.items():
            ...
            platform_button.clicked.connect(
                ...
                p=platform_name,
                img=image_data: self._on_platform_cover_clicked(p, img)
            )
    ...
    def _clear_platform_cover_buttons(self) -> None:
        ...
        for i in reversed(range(self.platform_covers_layout.count())):
            item = self.platform_covers_layout.itemAt(i).widget()
            if item != self.covers_label:  # Keep the label
                item.setParent(None)
                item.deleteLater()
    ...
    def _save_cover_image(self, track: Track, pixmap: QPixmap, metadata: dict) -> None:
