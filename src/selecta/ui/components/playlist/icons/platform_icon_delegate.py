...
class PlatformIconDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: "QStyleOptionViewItem", index: QModelIndex) -> None:
        ...
        model = index.model()
        if model is not None and hasattr(model, "force_refresh_data"):
            model.force_refresh_data = True  # type: ignore
        ...
        if model is not None:
            track_data = model.data(model.index(index.row(), 0), Qt.ItemDataRole.UserRole)
