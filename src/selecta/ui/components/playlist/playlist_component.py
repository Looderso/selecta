...
class PlaylistComponent(QWidget):
    ...
    def refresh(self) -> None:
        ...
        if (self.track_container.stacked_widget.currentWidget() == self.track_container.loading_widget or
            self.playlist_container.stacked_widget.currentWidget() == self.playlist_container.loading_widget):
    ...
    def _handle_refresh_finished_with_cleanup(self) -> None:
        ...
        if self.current_tracks and self.track_container.stacked_widget.currentWidget() != self.track_container.tracks_table:
    ...
    def _on_data_changed(self) -> None:
        ...
        if self.data_provider is not None and self.current_playlist_id is not None:
            # Debounce this refresh to prevent cascading updates
            if not hasattr(self, '_data_changed_timer'):
                from PyQt6.QtCore import QTimer
                self._data_changed_timer = QTimer()
                self._data_changed_timer.setSingleShot(True)
                self._data_changed_timer.timeout.connect(self._execute_data_changed_refresh)
                
            # Reset the timer if it's running
            if self._data_changed_timer.isActive():
                self._data_changed_timer.stop()
                
            # Start the timer with a delay (250ms)
            self._data_changed_timer.start(250)
    ...
    def _on_track_updated(self, track_id: int) -> None:
        ...
        if not hasattr(self, '_track_update_timers'):
            self._track_update_timers = {}
            
        # Cancel any existing timer for this track
        if track_id in self._track_update_timers and self._track_update_timers[track_id].isActive():
            self._track_update_timers[track_id].stop()
            
        # Create a new timer for this track update
        from PyQt6.QtCore import QTimer
        
        if track_id not in self._track_update_timers:
            self._track_update_timers[track_id] = QTimer()
            self._track_update_timers[track_id].setSingleShot(True)
            self._track_update_timers[track_id].timeout.connect(
                lambda: self._execute_track_update(track_id)
            )
            
        # Start the timer with a short delay (100ms)
        self._track_update_timers[track_id].start(100)
    ...
    def _ensure_loading_hidden(self) -> None:
        ...
        elif self.current_tracks and self.track_container.stacked_widget.currentWidget() != self.track_container.tracks_table:
        ...
        # Schedule another check after a small delay to catch any race conditions
        from PyQt6.QtCore import QTimer
        QTimer.singleShot(100, self._final_visibility_check)
    ...
    def _final_visibility_check(self) -> None:
        ...
        if (self.current_tracks and 
            self.track_container.stacked_widget.currentWidget() != self.track_container.tracks_table and
