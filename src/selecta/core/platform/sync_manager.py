"""Sync manager for platform synchronization operations."""
...
class PlatformSyncManager:
    """Manager for syncing tracks and playlists between platforms and local database.

    This class provides standardized methods for syncing operations that work with
    ...
    def sync_playlist(self, local_playlist_id: int) -> tuple[int, int]:
        """Sync a playlist bidirectionally between local database and platform.
        ...
        Raises:
            ValueError: If playlist cannot be synced
        """
        if not self.platform_client.is_authenticated():
            raise ValueError(f"{self.platform_name.capitalize()} client not authenticated")
        ...
        if playlist_name and playlist_name != local_playlist.name:
            self.playlist_repo.update(
                local_playlist_id, {"name": playlist_name, "last_synced": datetime.now(UTC)}
            )
        else:
            # Just update the sync timestamp
            self.playlist_repo.update(local_playlist_id, {"last_synced": datetime.now(UTC)})
